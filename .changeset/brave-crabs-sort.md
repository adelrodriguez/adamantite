...
Knip cannot see that `adamantite analyze` runs Sherif, and reports `sherif` as an unused devDependency. In a detected monorepo, `adamantite init` now writes `ignoreDependencies: ["sherif"]` to the generated `knip.config.ts`. Existing monorepo projects: add that line to your `knip.config.ts`.
