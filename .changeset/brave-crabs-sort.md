...
Knip cannot see that `adamantite analyze` runs Sherif, and reports `sherif` as an unused devDependency. In a detected monorepo, `adamantite init` now writes `ignoreDependencies: ignoreDependencies.monorepo` to the generated `knip.config.ts`, with the named `ignoreDependencies` import from `adamantite/analyze`. Existing monorepo projects: add that import and that line to your `knip.config.ts`.
