...
exits 0 only when no findings remain — the agent's exit code is ignored. When an agent
fails to start, the Markdown prompt copy remains available. Non-interactive output is
unchanged.
