...
describe("helpers integration", () => {
  ...
  describe("biome helper", () => {
    ...
    test("should update existing biome.jsonc config", async () => {
      ...
      await Bun.write("biome.jsonc", JSON.stringify({
    ...
    test("should handle biome.json (without c extension)", async () => {
      ...
      await Bun.write("biome.json", JSON.stringify({
        "rules": {
          "recommended": true
        }
      }, null, 2))
  ...
  describe("tsconfig helper", () => {
    ...
    test("should update existing tsconfig.json config", async () => {
      // Create initial config
      await Bun.write("tsconfig.json", JSON.stringify({
        "compilerOptions": {
          "target": "ES2020",
          "strict": true
        },
  ...
  describe("vscode helper", () => {
    ...
    test("should create .vscode directory and settings.json", async () => {
      ...
      expect(config["editor.codeActionsOnSave"]["quickfix.biome"]).toBe("explicit")
      expect(config["[javascript][typescript][javascriptreact][typescriptreact][json][jsonc][css][graphql]"]).toBeDefined()
      expect(config["[javascript][typescript][javascriptreact][typescriptreact][json][jsonc][css][graphql]"]["editor.defaultFormatter"]).toBe("biomejs.biome")
    })

    test("should update existing .vscode/settings.json config", async () => {
      // Create .vscode directory and initial settings
      mkdirSync(".vscode", { recursive: true })
      await Bun.write(".vscode/settings.json", JSON.stringify({
        "editor.fontSize": 14,
