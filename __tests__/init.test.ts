...
describe("init command", () => {
  ...
  test("sets up a fresh project with the selected files, scripts, and dependencies", async () => {
    ...
    const tsconfig = await readJson(join(tempDir, "tsconfig.json"))
  ...
  test("migrates a legacy oxlint config into oxlint.config.ts", async () => {
    await writeFile(
  ...
  test("uses workspace installation when the project is a monorepo", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify(
        {
    ...
    expect(installer.calls).toEqual([
      {
        ...
        packages: ["adamantite", "sherif@1.10.0"],
      },
    ])
  })

  test("updates existing configs without dropping preserved user settings", async () => {
  ...
  test("applies only the requested scripts and editor setup", async () => {
    const prompter = createPrompterTestContext({
      confirmResponses: [false, false],
    ...
    expect(installer.calls).toEqual([
      {
        options: { silent: true, workspace: false },
        packages: ["adamantite", "oxfmt@0.35.0"],
      },
    ])
  ...
  test("warns when both legacy and modern oxlint configs exist", async () => {
    await writeFile(
