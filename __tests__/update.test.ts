import { afterEach, beforeEach, describe, expect, test } from "bun:test"
import { mkdtempSync, rmSync } from "node:fs"
import { readFile, writeFile } from "node:fs/promises"
import { tmpdir } from "node:os"
import { join } from "node:path"
import * as Exit from "effect/Exit"
import updateCommand from "#commands/update.ts"
import {
  createDependencyInstallerTestContext,
  createPrompterTestContext,
  runCommand,
} from "./command-test-helpers.ts"

describe("update command", () => {
  let originalCwd: string
  let tempDir: string

  beforeEach(() => {
    originalCwd = process.cwd()
    tempDir = mkdtempSync(join(tmpdir(), "adamantite-update-test-"))
    process.chdir(tempDir)
  })

  afterEach(() => {
    process.chdir(originalCwd)
    rmSync(tempDir, { force: true, recursive: true })
  })

  test("reports when everything is already current and skips installation", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify(
        {
          devDependencies: {
            oxfmt: "0.35.0",
            oxlint: "1.50.0",
            "oxlint-tsgolint": "0.14.2",
            sherif: "1.10.0",
          },
          name: "test-project",
          version: "1.0.0",
        },
        null,
        2
      )
    )
    await writeFile(join(tempDir, "oxlint.config.ts"), "export default {}\n")

    const prompter = createPrompterTestContext()
    const installer = createDependencyInstallerTestContext()

    const exit = await runCommand(updateCommand, [], tempDir, [prompter.layer, installer.layer])

    expect(Exit.isSuccess(exit)).toBe(true)
    expect(installer.calls).toEqual([])
    expect(prompter.logs).toContainEqual({
      level: "success",
      message: "All adamantite dependencies are already up to date!",
    })
    expect(prompter.outros).toEqual(["✅ No updates needed"])
  })

  test("shows proposed dependency updates and installs exact target versions on confirmation", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify(
        {
          devDependencies: {
            oxfmt: "^0.34.0",
            oxlint: "~1.49.0",
            "oxlint-tsgolint": "workspace:^0.14.0",
            sherif: "1.9.0",
          },
          name: "test-project",
          version: "1.0.0",
        },
        null,
        2
      )
    )

    const prompter = createPrompterTestContext({
      confirmResponses: [true],
    })
    const installer = createDependencyInstallerTestContext()

    const exit = await runCommand(updateCommand, [], tempDir, [prompter.layer, installer.layer])

    expect(Exit.isSuccess(exit)).toBe(true)
    expect(installer.calls).toEqual([
      {
        options: { silent: true },
        packages: ["oxlint@1.50.0", "oxlint-tsgolint@0.14.2", "oxfmt@0.35.0", "sherif@1.10.0"],
      },
    ])
    expect(prompter.logs).toContainEqual({
      level: "info",
      message: "The following dependencies will be updated:",
    })
    expect(prompter.logs).toContainEqual({
      level: "info",
      message: "  oxlint: ~1.49.0 → 1.50.0",
    })
    expect(prompter.outros).toEqual(["✅ Dependencies updated successfully!"])
  })

  test("cancels dependency updates when the user declines", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify(
        {
          devDependencies: {
            oxlint: "1.0.0",
          },
          name: "test-project",
          version: "1.0.0",
        },
        null,
        2
      )
    )

    const prompter = createPrompterTestContext({
      confirmResponses: [false],
    })
    const installer = createDependencyInstallerTestContext()

    const exit = await runCommand(updateCommand, [], tempDir, [prompter.layer, installer.layer])

    expect(Exit.isSuccess(exit)).toBe(true)
    expect(installer.calls).toEqual([])
    expect(prompter.outros).toEqual(["⚠️ Update cancelled"])
  })

  test("migrates a legacy oxlint config even when no dependency updates are needed", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify(
        {
          devDependencies: {
            oxfmt: "0.35.0",
            oxlint: "1.50.0",
            "oxlint-tsgolint": "0.14.2",
            sherif: "1.10.0",
          },
          name: "test-project",
          version: "1.0.0",
        },
        null,
        2
      )
    )
    await writeFile(
      join(tempDir, ".oxlintrc.json"),
      JSON.stringify(
        {
          rules: {
            semi: "error",
          },
        },
        null,
        2
      )
    )

    const prompter = createPrompterTestContext()
    const installer = createDependencyInstallerTestContext()

    const exit = await runCommand(updateCommand, [], tempDir, [prompter.layer, installer.layer])

    expect(Exit.isSuccess(exit)).toBe(true)
    expect(installer.calls).toEqual([])
    expect(await Bun.file(join(tempDir, ".oxlintrc.json")).exists()).toBe(false)

    const oxlintConfig = await readFile(join(tempDir, "oxlint.config.ts"), "utf8")
    expect(oxlintConfig).toContain('"semi": "error"')
    expect(prompter.outros).toEqual(["✅ Oxlint config migrated to `oxlint.config.ts`!"])
  })

  test("warns when both oxlint config formats are present and prefers oxlint.config.ts", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify(
        {
          devDependencies: {
            oxlint: "1.50.0",
          },
          name: "test-project",
          version: "1.0.0",
        },
        null,
        2
      )
    )
    await writeFile(
      join(tempDir, "oxlint.config.ts"),
      'export default { rules: { curly: "error" } }\n'
    )
    await writeFile(
      join(tempDir, ".oxlintrc.json"),
      JSON.stringify(
        {
          rules: {
            semi: "error",
          },
        },
        null,
        2
      )
    )

    const prompter = createPrompterTestContext()
    const installer = createDependencyInstallerTestContext()

    const exit = await runCommand(updateCommand, [], tempDir, [prompter.layer, installer.layer])

    expect(Exit.isSuccess(exit)).toBe(true)
    expect(prompter.logs).toContainEqual({
      level: "warning",
      message:
        "Found both `oxlint.config.ts` and `.oxlintrc.json`. Adamantite will use `oxlint.config.ts`.",
    })
    expect(await Bun.file(join(tempDir, "oxlint.config.ts")).exists()).toBe(true)
    expect(await Bun.file(join(tempDir, ".oxlintrc.json")).exists()).toBe(true)
  })
})
