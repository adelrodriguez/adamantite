...
The `antislop` preset ships a vendored bundle of
[anti-slop](https://github.com/dmmulroy/anti-slop) at `presets/lint/antislop/plugin.mjs`
because upstream deliberately does not publish to npm. The bundle is generated by
`scripts/vendor-anti-slop.ts` from a pinned upstream commit, inlines the plugin's only
dependency, and is checked in with its license attribution. To update, bump `PINNED_REF`
in the script, re-run it, review the upstream diff, and adjust the rules in
`presets/lint/antislop.ts` if rules were added, removed, or renamed.
