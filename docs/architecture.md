...
A preset can ship a vendored bundle of a third-party plugin. The upstream either
deliberately does not publish to npm (anti-slop), or publishes a package whose
dependencies serve only another linter (`@shadcn/lint` and its ESLint peers). Bundles live under `presets/lint/vendor/`, are generated by
...
A bundle can have companion files that the plugin loads by URL at runtime, such as the
shadcn bundle's `tailwind-worker.js`. The script bundles each one on its own and writes it
beside `plugin.mjs`. List every file of a bundle in the `copy` entries of
`tsdown.config.ts`, or the published preset loses it.
