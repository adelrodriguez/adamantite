...
/**
 * Sherif CLI flags and the camelCase keys of the `sherif` field that replace them. `--fix` is not a
 * project setting, so it has no key.
 */
  ...
  return [
    {
      ...
      goal: [
        ...
        ...legacyScripts.flatMap((script) => getSettingGoals(script)),
